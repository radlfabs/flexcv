...
def test_preprocess_slopes_no_scaling():
    ...
    Z_train, Z_test = preprocess_slopes(Z_train_slope.copy(), Z_test_slope.copy(), must_scale=False)
...
def test_preprocess_slopes_with_scaling():
    ...
    Z_train_slope = pd.DataFrame(np.random.rand(10, 5))
    Z_test_slope = pd.DataFrame(np.random.rand(10, 5))
    Z_train, Z_test = preprocess_slopes(Z_train_slope.copy(), Z_test_slope.copy(), must_scale=True)
...
def test_cross_validate():
    ...
    mapping = ModelMappingDict({
        "RandomForestRegressor": {
            ...
            "params": {"n_estimators": optuna.distributions.IntUniformDistribution(10, 100)},
    ...
    for i in range(n_splits_out):
        assert isinstance(results["RandomForestRegressor"]["model"][i], RandomForestRegressor)
        ...
        assert isinstance(results["RandomForestRegressor"]["parameters"][i]["n_estimators"], int)
        assert isinstance(results["RandomForestRegressor"]["parameters"][i]["random_state"], int)
        
        assert len(results["RandomForestRegressor"]["parameters"][i]) == 2
        
        assert len(results["RandomForestRegressor"]["y_pred"][i]) == len(results["RandomForestRegressor"]["y_test"][i])
        assert len(results["RandomForestRegressor"]["y_pred_train"][i]) == len(results["RandomForestRegressor"]["y_train"][i])
