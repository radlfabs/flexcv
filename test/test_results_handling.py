...
def test_add_summary_stats():
    ...
    expected_result = pd.DataFrame(
        {"column1": [1, 2, 3, 2, 2, 1], "column2": [4, 5, 6, 5, 5, 1]},
        index=[0, 1, 2, "mean", "median", "std"],
    )
...
def test_cross_validation_results_init():
    ...
    results_dict = {
        "Model1": {
            "metrics": [{"metric1": 1, "metric2": 2}, {"metric1": 3, "metric2": 4}]
        },
        "Model2": {
            "metrics": [{"metric1": 5, "metric2": 6}, {"metric1": 7, "metric2": 8}]
        },
    }
...
def test_get_best_model_by_metric_specific_model():
    ...
    results = CrossValidationResults(results_dict)
    best_model = results.get_best_model_by_metric(
        model_name="Model2", metric_name="mse", direction="min"
    )
...
def test_get_predictions():
    ...
    results_dict = {"Model1": {"y_pred": [[1, 2, 3], [4, 5, 6], [7, 8, 9]]}}
    results = CrossValidationResults(results_dict)
...
def test_get_true_values():
    ...
    results_dict = {"Model1": {"y_test": [[1, 2, 3], [4, 5, 6], [7, 8, 9]]}}
    results = CrossValidationResults(results_dict)
...
def test_get_training_predictions():
    ...
    results_dict = {"Model1": {"y_pred_train": [[1, 2, 3], [4, 5, 6], [7, 8, 9]]}}
    results = CrossValidationResults(results_dict)
    training_predictions = results.get_training_predictions(
        model_name="Model1", fold_id=1
    )
...
def test_get_training_true_values():
    ...
    results_dict = {"Model1": {"y_train": [[1, 2, 3], [4, 5, 6], [7, 8, 9]]}}
    results = CrossValidationResults(results_dict)
    training_true_values = results.get_training_true_values(
        model_name="Model1", fold_id=1
    )
