...
def test_custom_scorer_different_values():
    ...
    expected_result = (
        ...
        + 0.5 * abs(mean_squared_error(y_train_in, y_pred_train) - mean_squared_error(y_valid, y_pred))
        + 2 * max(0, (mean_squared_error(y_train_in, y_pred_train) - mean_squared_error(y_valid, y_pred) - 0.05))
        + 1 * max(0, -(mean_squared_error(y_train_in, y_pred_train) - mean_squared_error(y_valid, y_pred)))
    )
...
def test_objective():
    ...
    score_valid, score_train, score_of = objective(X_train_in, y_train_in, X_valid, y_valid, pipe, params, objective_scorer)
...
def test_parallel_objective():
    ...
    score_valid, score_train, score_of = parallel_objective(train_idx, valid_idx, X, y, pipe, params_, objective_scorer)
    ...
    expected_score_of = -valid_scorer(y.iloc[valid_idx], y_pred, y.iloc[train_idx], y_pred_train)
...
def test_objective_cv_sequential():
    ...
    pipe = Pipeline([('regressor', LinearRegression())])
    params = {'regressor__fit_intercept': {'name': 'regressor__fit_intercept', 'type': 'categorical', 'choices': [True, False]}}
    X = pd.DataFrame({'feature': [1, 2, 3, 4, 5]})
    y = pd.Series([1, 2, 3, 4, 5])
    run = MagicMock()
    n_jobs = 1
    ...
    result = objective_cv(trial, cross_val_split, pipe, params, X, y, run, n_jobs, objective_scorer)
...
def test_objective_cv_same_results_sequential_parallel():
    ...
    result_sequential = objective_cv(trial, cross_val_split, pipe, params, X, y, run, 1, objective_scorer)
    result_parallel = objective_cv(trial, cross_val_split, pipe, params, X, y, run, -1, objective_scorer)
