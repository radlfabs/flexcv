...
def test_make_cross_val_split_contistrat():
    ...
    X, y = make_classification(n_samples=100, n_features=20, n_informative=2, n_redundant=10, random_state=42)
    groups = np.random.choice([1, 2, 3, 4, 5], size=100)
    cv = make_cross_val_split(method=CrossValMethod.CONTISTRAT, n_splits=5, groups=groups, random_state=42)
    splits = list(cv(X, y))
    # Check that the correct number of splits are returned
    assert len(splits) == 5
    # Check that the training and test sets are disjoint
...
def test_make_cross_val_split_contigroup():
    ...
    X, y = make_classification(n_samples=100, n_features=20, n_informative=2, n_redundant=10, random_state=42)
    groups = np.random.choice([1, 2, 3, 4, 5], size=100)
    cv = make_cross_val_split(method=CrossValMethod.CONTISTRATGROUP, n_splits=5, groups=groups, random_state=42)
    splits = list(cv(X, y))
    # Check that the correct number of splits are returned
    assert len(splits) == 5
    # Check that the training and test sets are disjoint
